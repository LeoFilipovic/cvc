...
/* CUDA 2P2 BREAKDOWN */
__host__ void compute_2p2_gpu(double *fwd_y, double *P1, double *P23, int iflavor, int const gsw[4], 
     const int *gycoords, int n_y, const double xunit[2], QED_kernel_temps kqed_t,  unsigned VOLUME, 
...
__global__ void kernel_p1(double *Pi, double *P1, int iflavor,  int const * gsw, unsigned VOLUME, 
     int const g_proc_coords[4], unsigned T, unsigned LX, unsigned LY, unsigned LZ, 
     unsigned T_global, unsigned LX_global, unsigned LY_global, unsigned LZ_global);
__global__ void kernel_p23(double *pi, double *P23, int n_y, const int*gsw, 
     const int *gycoords, const double xunit[2], QED_kernel_temps kqed_t, unsigned VOLUME, 
     int const g_proc_coords[4], unsigned T, unsigned LX, unsigned LY, unsigned LZ, 
     unsigned T_global, unsigned LX_global, unsigned LY_global, unsigned LZ_global);
