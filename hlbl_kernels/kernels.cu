...
QED_kernel_temps kqed_t, unsigned VOLUME, const int g_proc_coords[4]){
    const int n_p23 = n_y * kernel_n * kernel_n_geom * 4 * 4 * 4;
    //set_zero(P23, n_p23);
    

    for ( int yi = blockIdx.x /* * blockDim.x + threadIdx.x */; yi < n_y; yi+=gridDim.x /* * blockDim.x */ ){
        // For P2: y = (gsy - gsw)
        // For P3: y' = (gsw - gsy)
        ...
        for (int ix = 0; ix<VOLUME; ix ++){
            ...
            int const x[4] = {(ix / (LX * LY * LZ) - gsw[0] + g_proc_coords[0] * T + T_global) % T_global,
...
__host__ void compute_2p2_gpu(double *fwd_y, double *P1, double *P23, int iflavor, 
     int const gsw[4], int *gycoords, int n_y, const double xunit[2], 
     QED_kernel_temps kqed_t,  unsigned VOLUME, int const g_proc_coords[4]) {
    ...
    // Allocation
    size_t size_pi = sizeof(double) * 4 * 4 * 4 * VOLUME;
    size_t size_fwd = sizeof(double) * 2 * 12 * 24 * VOLUME;
    size_t size_p23 = sizeof(double) * n_y * kernel_n * kernel_n_geom * 4 * 4 * 4;
    size_t size_p1  = sizeof(double) * 4 * 4 * 4 * T_global;

    cudaMalloc((void **)&Pi_d, size_pi);
    cudaMalloc((void **)&fwd_y_d, size_fwd);
    cudaMalloc((void **)&P23_d, size_p23);
    cudaMalloc((void **)&P1_d, size_p1);
    cudaMalloc((void **)&gycoords_d, sizeof(int) * 4 * n_y);

    // Async copies (using streams to overlap transfer if needed, though usually fast)
    cudaMemcpyAsync(fwd_y_d, fwd_y, size_fwd, cudaMemcpyHostToDevice, stream_p23);
    cudaMemcpyAsync(gycoords_d, gycoords, sizeof(int) * 4 * n_y, cudaMemcpyHostToDevice, stream_p23);
    ...
    // We can free fwd_y now, but be careful with stream safety. 
    // Best to sync or wait until end if VRAM allows. 
    // On GH200 VRAM is usually plentiful (96GB+), so let's free at the end to avoid syncs here.

    /* --- 3. CONCURRENT KERNEL LAUNCH --- */
    
    // --- Stream 1: P23 ---
    dim3 gridP23(128);
    dim3 blockP23(kernel_n);
    kernel_p23<<<gridP23, blockP23, 0, stream_p23>>>(
        Pi_d, P23_d, n_y, kernel_n, kernel_n_geom, gsw, gycoords_d, xunit, kqed_t, VOLUME, g_proc_coords
    );

    // --- Stream 2: P1 ---
    dim3 gridP1(4, 4, 4);
    dim3 blockP1(64);
    kernel_p1<<<gridP1, blockP1, 0, stream_p1>>>(
        Pi_d, P1_d, iflavor, gsw, VOLUME, g_proc_coords[0], g_proc_coords[1], g_proc_coords[2], g_proc_coords[3]
    );
    ...
    MPI_Iallreduce(MPI_IN_PLACE, P23_d, size_p23/sizeof(double), MPI_DOUBLE, MPI_SUM, g_cart_grid, &request[0]);

    // Wait for P1 Kernel to finish, then start its MPI
    cudaStreamSynchronize(stream_p1);
    MPI_Iallreduce(MPI_IN_PLACE, P1_d, size_p1/sizeof(double), MPI_DOUBLE, MPI_SUM, g_cart_grid, &request[1]);
    ...
    cudaMemcpy(P23, P23_d, size_p23, cudaMemcpyDeviceToHost);
    cudaMemcpy(P1, P1_d, size_p1, cudaMemcpyDeviceToHost);
...
__host__ void record_p23_cuda(double *Pi, int n_y, int kernel_n, int kernel_n_geom, const int gsw[4], const int *gycoords, const double xunit[2], QED_kernel_temps kqed_t, unsigned VOLUME) {
    // set up on gpu
    double *Pi_d, *P23_d;
    cudaMalloc((void **)&Pi_d, sizeof(double) * 16 * VOLUME);
    cudaMemcpy(Pi_d, Pi, sizeof(double) * 16 * VOLUME, cudaMemcpyHostToDevice);
    ...
    // compute p23 on gpu
    kernel_p23<<<gridDim, blockDim>>>(Pi_d, P23_d, n_y, kernel_n, kernel_n_geom, gsw, gycoords_d, xunit, kqed_t, VOLUME, g_proc_coords);
 ...
 */
    ...
    //cudaSetDevice(rank); // bind GPU to rank?
    //if (g_cart_id == 0) 
    ...
    initialise(&kqed_t);
    //compute_2p2_gpu(fwd_y, P1, P23, 0, gsw, gycoords, n_y, xunit, kqed_t, VOL, g_proc_coords);
    //record_pi_cuda(fwd_y, VOL, 0);
    double *Pi = (double *) malloc(sizeof(double) * 16 * VOL);
    srand(1234);
    for (int i=0; i<16 * VOL; i++) Pi[i] = rand()*2./RAND_MAX - 1.;
    //record_p23_cuda(Pi, n_y, kernel_n, kernel_n_geom, gsw, gycoords, xunit, kqed_t, VOL);
    //record_p1_cuda(Pi, 0, gsw, VOL);
